...
{
    int low = 0, high = node->car_num_keys - 1, mid;

    // Binary search within the node
    while (low <= high) {
        mid = (low + high) / 2;
        int cmp = strcmp(VIN, node->car_data[mid].car_VIN);
    ...
    // Recurse into the correct child (low is the right child index now)
    if (node->isleaf)
        return 0;

    return car_exists(node->children[low], VIN);
}
...
int sell_car_to_customer(showroom *target_showroom,  char *VIN_to_sell) {
    ...
    delet_car(target_showroom->avialable_cars_root->car_children, VIN_to_sell);
...
void predict_next_month_sales(car_node *sold_cars_root) {
    ...
    //Traverse the B-tree and update the sales count for cars sold in the previous month
...
typedef struct hash_entry2 {
    char VIN[SIZE];
    sold_car_info car;
    struct hash_entry2 *next;
} hash_entry2;


hash_entry2 *emi_car_hash_table[HASH_SIZE];
...
void init_emi_car_hash_table() {
    for (int i = 0; i < HASH_SIZE; i++) {
        emi_car_hash_table[i] = NULL;
    }
}

void insert_into_emi_car_hash_table(const sold_car_info *car) {
    if (!car || car->loan_period < 36 || car->loan_period > 48) return;

    unsigned int index = hash_function2(car->VIN);
    hash_entry2 *new_entry = (hash_entry2 *)malloc(sizeof(hash_entry2));
    if (!new_entry) {
        fprintf(stderr, "Memory allocation failed for hash entry.\n");
        return;
    }

    strcpy(new_entry->VIN, car->VIN);
    new_entry->car = *car;
    new_entry->next = emi_car_hash_table[index];
    emi_car_hash_table[index] = new_entry;
}

sold_car_info *find_emi_car_by_VIN(const char *VIN) {
    unsigned int index = hash_function2(VIN);
    hash_entry2 *curr = emi_car_hash_table[index];
    while (curr) {
        if (strcmp(curr->VIN, VIN) == 0) {
            return &curr->car;
        }
        curr = curr->next;
    }
...
void free_emi_car_hash_table() {
    for (int i = 0; i < HASH_SIZE; i++) {
        hash_entry2 *curr = emi_car_hash_table[i];
        while (curr) {
            hash_entry2 *temp = curr;
            curr = curr->next;
            free(temp);
        }
        emi_car_hash_table[i] = NULL;
...
void build_emi_car_hash_table(sold_car_node *root) {
    if (!root) return;
    for (int i = 0; i < root->sold_car_num_keys; i++) {
        insert_into_emi_car_hash_table(&root->sold_car[i]);
    }
    if (!root->isleaf) {
        for (int i = 0; i <= root->sold_car_num_keys; i++) {
            build_emi_car_hash_table(root->children[i]);
        }
    }
}

void print_customers_with_emi(customer_node *root) {
    if (!root) return;

    for (int i = 0; i < root->customer_num_keys; i++) {
        customer *cust = &root->customer_data[i];
        sold_car_info *car = find_emi_car_by_VIN(cust->sold_car_VIN);
        if (car) {
            printf("Customer Name   : %s\n", cust->customer_name);
            printf("Customer ID     : %s\n", cust->customer_id);
            printf("Mobile No       : %s\n", cust->mobile_no);
    ...
    if (!root->isleaf) {
        for (int i = 0; i <= root->customer_num_keys; i++) {
            print_customers_with_emi(root->children[i]);
        }
    }
}


void find_customers_with_emi_range(sold_car_node *sold_car_root, customer_node *customer_root) {
    if (!sold_car_root || !customer_root) {
        fprintf(stderr, "Error: Sold car tree or customer tree is NULL.\n");
        return;
    }

    // Step 1: Clean and initialize the hash table
    init_emi_car_hash_table();

    // Step 2: Build hash table from sold car tree
    build_emi_car_hash_table(sold_car_root);

    // Step 3: Traverse and print matching customers
    printf("Customers with EMI plan between 36 and 48 months:\n");
    printf("=================================================\n");
    print_customers_with_emi(customer_root);

    // Step 4: Free hash table memory
    free_emi_car_hash_table();
}
...
void traverse_salesperson_and_print(sales_person_node *salesperson_root) {
    ...
    for (int i = 0; i < salesperson_root->sales_person_num_keys; i++) {
        ...
        if (salesperson.sold_car_root && salesperson.customer_tree_root->customer_children) {
            ...
            find_customers_with_emi_range(salesperson.sold_car_root, salesperson.customer_tree_root->customer_children);
        }
    }
